...
describe("the NOW marker", () => {
  it("carries no text, so it only has to clear the graphic threshold", () => {
    // The marker used to be a labelled pill and needed 4.5:1 for its own text.
    // It is a bare 2px rule now: the label was redundant beside a chart whose
    // axis already says what time it is. Nothing may reintroduce text on it
    // without checking — near-black on the Dayglass orange is 4.39:1 and would
    // fail.
    for (const block of Object.values(THEMES)) {
      expect(() => token(block, "--wm-now-text")).toThrow();
    }
  });

  it("keeps the accent readable under the text that sits on it", () => {
    // The TODAY tag and the LIVE button are solid accent with page-coloured
    // text — the pairing the design uses wherever the accent is a fill.
    for (const [name, block] of Object.entries(THEMES)) {
      const bg = toRgb(token(block, "--wm-accent"));
      const fg = toRgb(token(block, "--wm-page"), bg);
      expect(contrast(fg, bg), `${name} accent fill`).toBeGreaterThanOrEqual(4.5);
    }
  });

  it("is clearly not the accent, so position never reads as quality", () => {
    // The accent means "worth acting on". A now marker in the same hue would
    // make the strip say two things with one signal.
    for (const block of Object.values(THEMES)) {
  ...
  it("shares its hue with marginal, deliberately", () => {
    // These were separate hues (orange and magenta) and the magenta read as an
    // error state next to the cyan. They are the same colour now because they
    // never appear as the same mark: marginal is a fill on a bar or a ring,
    // the now marker is a 2px vertical rule. Encoded as a test so a future
    // "let's differentiate these" does not quietly undo the decision.
    for (const [name, block] of Object.entries(THEMES)) {
      expect(toRgb(token(block, "--wm-now")), `${name}`).toEqual(
        toRgb(token(block, "--wm-marginal"))
      );
    }
  });
});
...
describe("opacity-capable tokens", () => {
  // Tailwind opacity modifiers (bg-caution/10) only work against a channel
  // triple. Declared as a composite var they emit invalid CSS and the property
  // silently falls back — which is how MAYBE ended up with a white border.
  it("declares every token used with an opacity modifier as a channel triple", () => {
    for (const [name, block] of Object.entries(THEMES)) {
      for (const token of ["--wm-page", "--wm-ink", "--wm-accent", "--wm-marginal", "--wm-caution"]) {
        const value = block.match(new RegExp(`${token}:\\s*([^;]+);`))[1].trim();
        expect(value, `${name} ${token}`).toMatch(/^\d+ \d+ \d+$/);
      }
    }
  });
});
