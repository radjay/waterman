...
export function buildBaselinePrediction({
  ...
  const usablePoints = points.filter(isUsableForecastPoint);
  const byValidTime = groupBy(usablePoints, (point) => point.validTime);
  ...
  return {
    ...
    thresholdKnots,
    predictedKickInAt: firstLikely?.time,
    predictedStrongKickInAt: firstHigh?.time,
