import { getLiveWindDirectionLabel } from "./utils";
 ...
 * stacked grey bars; live station stays as accent lines on top.
 */
...
export function buildStationCard({
  ...
  return {
    ...
    // Raw meteorological FROM bearing — LiveStationBadge recomputes the label
    // via getLiveWindDirectionLabel so a stale directionLabel cannot slip through.
    direction: Number.isFinite(latest.direction) ? latest.direction : null,
    // Windguru FROM → same FROM label forecast slots print (via TO storage +
    // getDisplayWindDirection). Rendering the raw FROM cardinal here made live
    // badges disagree with WindLine by 180°.
    directionLabel: Number.isFinite(latest.direction)
      ? getLiveWindDirectionLabel(latest.direction)
      : null,
