...
describe("windHoverRows", () => {
  it("spells gusts out as their own row, not a parenthetical", () => {
...
describe("columnHoverCard", () => {
  ...
  it("stacks forecast and station when both exist", () => {
    const station = {
    ...
    expect(hoverCardText(card)).toContain("station 6 kt");
    expect(hoverCardText(card)).toContain("gusts 9 kt");
    expect(hoverCardText(card)).toContain("forecast 12 kt");
  });

  it("omits the station rows when there is no reading", () => {
    const card = columnHoverCard(column, null, T0 + HOUR);
    expect(card.time).toBe("10am");
    expect(card.rows.map((r) => r.label)).toEqual(["forecast", "gusts"]);
    expect(columnHoverText(column, null, T0 + HOUR)).toBe(
      "10am · forecast 12 kt · gusts 16 kt"
    );
  });
});
...
describe("stationHoverCard", () => {
  it("leads with the sample clock, not the slot hour", () => {
    ...
    expect(card.rows[0]).toMatchObject({ label: "station", value: "6 kt" });
...
describe("resolveChartHover", () => {
  ...
  it("prefers a nearby station sample over the 3h column", () => {
    ...
    expect(hit.card.rows[0]).toMatchObject({ label: "station", value: "6 kt" });
  ...
  it("falls back to the forecast column away from station points", () => {
    ...
    expect(hit.card.rows.some((r) => r.label === "forecast")).toBe(true);
  ...
  it("marks the in-slot station sample on a column hit that shows it in the tip", () => {
    ...
    const hit = resolveChartHover({ chart, station, xPct, nowMs });
    expect(hit.kind).toBe("column");
    expect(hit.card.rows.some((r) => r.label === "station" && r.value === "6 kt")).toBe(
      true
    );
