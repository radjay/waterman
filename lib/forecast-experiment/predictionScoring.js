...
export function isPredictedRideable(prediction) {
  return getPredictedKickInAt(prediction) != null;
}
...
export function resolvePredictionMode(prediction) {
  return prediction?.mode ?? prediction?.inputs?.mode ?? null;
}

export function selectDayPrediction(
  predictions,
  { forecastDateLocal, modelVersion, thresholdKnots, mode, preferLatest = false }
) {
  ...
  if (mode) {
    const modeMatches = candidates.filter((prediction) => resolvePredictionMode(prediction) === mode);
  ...
  return candidates.sort((a, b) =>
    preferLatest ? b.generatedAt - a.generatedAt : a.generatedAt - b.generatedAt
  )[0];
}

export function predictionModesForModel(predictions, modelVersion) {
  const modes = new Set(
    predictions
      .filter((prediction) => prediction.modelVersion === modelVersion)
      .map((prediction) => resolvePredictionMode(prediction))
      .filter(Boolean)
  );
  if (modes.size === 0) {
    return [null];
...
export function scorePredictionDay({ label, prediction }) {
  ...
  const kickInErrorMinutes = computeErrorMinutes(label.actualKickInAt, getPredictedKickInAt(prediction));
  ...
  return {
    ...
    predictionMode: resolvePredictionMode(prediction),
  };
}
