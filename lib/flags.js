...
export const FLAGS = {
  ...
  /**
   * Station sparkline and the "+2 vs forecast" pill on the Now card. Also
   * gates whether a station reading is allowed to move the go/no-go verdict
   * (lib/verdict.js deriveVerdict, via stationDelta) — NO to MARGINAL is a
   * real change in the advice a rider is given, not just a UI reveal.
   *
   * OFF until a live delta has been eyeballed on a spot whose station is
   * genuinely at the spot. Flip it on only after watching that comparison
   * hold up, not on a timer or a row count.
   */
  stationEvidence: {
    env: process.env.NEXT_PUBLIC_FLAG_STATION_EVIDENCE,
    description: "Live station reading, delta, and its vote in the go/no-go verdict",
  },
