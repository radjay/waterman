import { getDisplayWindDirection } from "../utils.js";
...
export function describeBayDay(
  ...
  const kickInTimePlain = kickInMs ? formatLisbonTime(kickInMs) : null;
  const kickInTime = formatKickInDisplay(kickInMs, sessionProbability);
  ...
  if (isLive && dateLocal === todayKey) {
    ...
    if (kickInMs && kickInMs > referenceMs) {
      ...
      return {
        ...
        kickInTime,
        kickInTimePlain,
        kickInAtMs: verdict === "skip" ? null : kickInMs,
        likelihoodPct,
        dayLabel,
        dateLocal,
      };
  ...
  return {
    ...
    kickInAtMs: verdict === "skip" ? null : kickInMs ?? null,
    likelihoodPct,
    dayLabel,
    dateLocal,
  };
...
export function describeCaboLine(observation) {
  ...
  const direction =
    observation.windDirectionDeg != null
      ? getDisplayWindDirection(observation.windDirectionDeg)
      : null;
