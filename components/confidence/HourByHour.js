 ...
 */
export function HourByHour({ slots, sport, tides = [], peakTimestamp = null }) {
    ...
    <section>
      ...
      <div className="rounded-[15px] bg-surface border border-card overflow-hidden">
        {slots.map((slot, i) => {
          ...
          const isPeak = peakTimestamp != null && slot.timestamp === peakTimestamp;

          const turns = tides.filter(
            (t) =>
              t.time >= slot.timestamp &&
              t.time < slot.timestamp + 3 * 60 * 60 * 1000
          );
              ...
              <span className="flex-1 min-w-0">
                ...
                {turns.length > 0 && (
                  <span className="block font-data text-[9px] text-dim mt-0.5">
                    {turns
                      .map((t) => `${t.type.toUpperCase()} TIDE ${time(t.time)}`)
                      .join(" · ")}
                  </span>
                )}
              </span>
