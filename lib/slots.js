 ...
 * @returns {Array} Enriched slots with matching criteria, sport, score, etc.
 */
export function enrichSlots(slots, spot, configs, scoresMap = {}, relevantSports = []) {
  return slots.map((slot) => {
    ...
    // Find score for this slot (check all relevant sports, prefer matched sport)
    let score = null;
    const sportsToCheck = matchedSport ? [matchedSport, ...relevantSports.filter(s => s !== matchedSport)] : relevantSports;
