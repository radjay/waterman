...
// Helper to find Chrome executable on Render
function findChromeExecutable() {
    ...
    try {
        ...
        // Puppeteer couldn't find it, continue to manual search
    }
        ...
        if (path.includes('*')) {
            ...
            try {
                ...
                if (fs.existsSync(parentDir)) {
                    ...
                    for (const d of dirs) {
                        const fullPath = `${parentDir}/${d}/${pattern}`;
...
// Helper to install Chrome if not found
async function ensureChromeInstalled() {
    ...
    try {
        ...
        const newPath = findChromeExecutable();
        if (newPath) {
...
export async function getForecast(spotUrl, spotId = null) {
    ...
    if (isRender) {
        ...
        // Ensure Chrome is installed and find executable path
        ...
        if (chromePath) {
            ...
            console.warn('Warning: Could not find or install Chrome executable. Puppeteer will try to use default.');
        }
    }
    ...
    try {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                ...
                await page.waitForSelector('#windywidgettable', { timeout: 45000 });
                tableFound = true;
                break;
            } catch (error) {
                if (attempt < 2) {
                    console.log(`Table not found, attempt ${attempt + 1}/3, waiting and retrying...`);
                    ...
                    throw new Error(`Could not find windy widget table after 3 attempts. Page title: ${pageContent.title}`);
                }
            }
        }
