...
import { isDaylightSlot, isAfterSunset } from "./daylight";
...
export function markContextualSlots(grouped, spotsMap, selectedSports) {
  Object.keys(grouped).forEach((day) => {
    Object.keys(grouped[day]).forEach((spotId) => {
      ...
      // Mark slots as contextual only if they don't overlap with daylight at all.
      // Slots where sunrise or sunset falls within the 3h window are treated as normal daylight.
      slots.forEach(slot => {
        if (!isDaylightSlot(new Date(slot.timestamp), spot)) {
