...
describe("deriveVerdict", () => {
  ...
  it("forces NO after sunset / before sunrise even on a strong score", () => {
    // You cannot ride in the dark — night wins over wind/wave.
    expect(
      deriveVerdict({ score: 92, agreement: agreement(5), isDark: true })
    ).toBe(VERDICT.NO);
    expect(
      deriveVerdict({ score: 60, agreement: agreement(5), laterPeak: 90, isDark: true })
    ).toBe(VERDICT.NO);
  });

  it("still scores normally in daylight when isDark is false", () => {
    expect(
      deriveVerdict({ score: 92, agreement: agreement(5), isDark: false })
    ).toBe(VERDICT.GO);
  });
