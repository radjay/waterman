...
describe("buildStationCard", () => {
  ...
  it("reports the newest reading", () => {
    ...
    // Raw FROM degrees are exposed so LiveStationBadge can always re-flip.
    expect(card.direction).toBe(315);
    // 315° FROM (NW) → same FROM label forecast prints beside it (NNW/NW).
