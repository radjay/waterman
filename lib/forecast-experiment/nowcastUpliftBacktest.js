...
export function summarizeNowcastUplift(days, { minMeanUpliftMinutes = 15, minImprovedShare = 0.5 } = {}) {
  ...
  const comparable = qualifying.filter(
    (day) =>
      ...
      day.forecast?.predictedKickInAt != null &&
      day.nowcast?.predictedKickInAt != null
  );
...
export function classifyUpliftDayRegime({
  dateLocal,
  actual,
  caboObservations,
  marinaObservations = [],
...
export async function loadNowcastUpliftSeasonData(
  ...
  return {
    ...
    marinaByDate: groupObservationsByDate(marinaObservations),
  };
}
...
export function buildNowcastUpliftDays({
  ...
  return datesLocal.map((dateLocal) => {
    ...
    const regime = classifyUpliftDayRegime({
      ...
      actual: comparison.actual,
      caboObservations,
      marinaObservations: marinaByDate?.get(dateLocal) ?? [],
      thresholdKnots,
    });
