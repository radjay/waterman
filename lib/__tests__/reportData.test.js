  ...
  data: {
    spotA: {
      ...
      scoresMap: {
        ...
        "3000_wingfoil": {
          score: 88,
          reasoning: "Nortada fills in through the afternoon.",
          factors: { windQuality: 90, waveQuality: 40, tideQuality: 60 },
        },
...
describe("slotsForSpot", () => {
  ...
  it("carries the scorer's reasoning and factors, which used to be dropped", () => {
    // Both are stored on every score row. The adapter silently discarded them,
    // so the Confidence screen had nothing to show without per-model data.
    const slot = slotsForSpot(report, "spotA", "wingfoil").find((s) => s.timestamp === 3000);
    expect(slot.reasoning).toContain("Nortada");
    expect(slot.factors.windQuality).toBe(90);
  });

  it("uses null for reasoning and factors when the score row lacks them", () => {
    const slot = slotsForSpot(report, "spotA", "wingfoil").find((s) => s.timestamp === 1000);
    expect(slot.reasoning).toBeNull();
    expect(slot.factors).toBeNull();
  });
