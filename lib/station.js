...
/**
 * Paint the model forecast onto each history sample: the speed of the forecast
 * slot covering that timestamp. Step-shaped on the chart so you can see when
 * the model held a level vs when the station wandered.
 */
export function attachForecast(history, forecastSlots, slotMs = FORECAST_SLOT_MS) {
  ...
  if (slots.length === 0) {
    return history.map((p) => ({ ...p, forecast: p.forecast ?? null }));
  }
...
export function buildStationCard({
  ...
  const history = attachForecast(bucketHistory(sorted, nowMs), slotsForChart);
