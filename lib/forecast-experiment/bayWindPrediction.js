...
export function buildBayWindPredictionV2({
  ...
  let kickInP50At = firstLikely?.time;
  if (mode === "nowcast" && caboKickInAt != null && caboLagMinutes != null) {
    const inferredKickIn = caboKickInAt + caboLagMinutes * 60_000;
    if (!kickInP50At || inferredKickIn < kickInP50At) {
      kickInP50At = inferredKickIn;
    }
  }

  let kickInP75At = firstHigh?.time;
  if (mode === "nowcast" && caboKickInAt != null && caboLagMinutes != null && kickInP50At) {
    const inferredKickIn = caboKickInAt + caboLagMinutes * 60_000;
    if (!kickInP75At || inferredKickIn < kickInP75At) {
      kickInP75At = inferredKickIn;
    }
  }
  ...
  return {
    ...
    confidence,
    summary: summaryV2({
      firstLikely: kickInP50At ? { time: kickInP50At } : undefined,
      firstHigh: kickInP75At ? { time: kickInP75At } : undefined,
      peak,
      latestCabo,
      caboLagMinutes,
      mode,
