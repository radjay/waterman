...
describe("upcomingWindows", () => {
  ...
  it("drops the running window at the spot the verdict already describes", () => {
    const rows = upcomingWindows(
      [
        { spot: A, windows: detectWindows([slot(9, 90), slot(18, 85)]) },
        { spot: B, windows: detectWindows([slot(9, 80)]) },
      ],
      at(10),
      3,
    ...
    // A's 09:00 window is live at 10:00 and belongs to the verdict card, so it
    // must not appear again. A's later window and B's live one are both real
    // options and stay.
    expect(rows.map((r) => `${r.spot._id}@${new Date(r.window.start).getUTCHours()}`)).toEqual([
      "b@9",
      "a@18",
    ]);
  });
...
describe("dayStartOf", () => {
  it("is the same for every moment within a Lisbon day", () => {
    // Now and Next both build the `day` segment of /window/[day]/[start] from
    // this. They used to hold private copies and drifted apart.
    const morning = Date.UTC(2026, 6, 14, 8, 0);
    const evening = Date.UTC(2026, 6, 14, 20, 0);
    expect(dayStartOf(morning)).toBe(dayStartOf(evening));
  });

  it("rolls over at local midnight, not UTC midnight", () => {
    // Lisbon is UTC+1 in July, so 23:30 UTC is already the next local day.
    const lateUtc = Date.UTC(2026, 6, 14, 23, 30);
    const earlyUtc = Date.UTC(2026, 6, 14, 12, 0);
    expect(dayStartOf(lateUtc)).not.toBe(dayStartOf(earlyUtc));
  });
});
