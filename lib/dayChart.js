 ...
 *
 * Callers with short bands (Report expanded days, Live sparklines) pass a lower
 * `maxLines` so 5/10/15/20/25 does not stack on a ~40px plot.
 */
 ...
 * denser set would crowd the band. Pass `maxLines` (e.g. 3) for short charts.
 */
export function windScale(values = [], { min = 30, step = 10, maxLines = 5 } = {}) {
  const finite = values.filter((v) => Number.isFinite(v));
  const peak = finite.length ? Math.max(...finite) : 0;
  const max = peak <= min ? min : niceCeil(peak * 1.05, step);
  return {
    max,
    lines: scaleLines(max, { tick: 5, format: (v) => `${v}kt`, maxLines }),
  };
}
 ...
 *
 * Pass `maxLines` for short bands (Report); tall Now charts keep the default.
 */
export function waveScale(values = [], { min = 1, step = 0.5, maxLines = 4 } = {}) {
  const finite = values.filter((v) => Number.isFinite(v));
  const peak = finite.length ? Math.max(...finite) : 0;
  const max = Math.max(min, niceCeil(peak * 1.25, step));
  return {
    max,
    lines: scaleLines(max, { tick: step, format: waveLabel, maxLines }),
  };
}
...
/**
 * Score bands, shared by the dial, the score bars and the Now verdict.
 *
 * Thresholds match deriveVerdict's score→word mapping so one reading never
 * paints MAYBE in caution beside a teal ring, or a teal bar under an orange
 * number:
 *
 *   <60   marginal @ .8   — NO
 *   60-74 caution  @ .55  — MAYBE (same hue as the verdict)
 *   75-85 accent   @ .62  — GO
 *   >85   accent   @ 1    — GO worth driving for
 */
...
export const SCORE_FILL = {
  ...
  // Caution (not accent): 60-74 is MAYBE on the verdict, so the bar must share
  // that band. Accent here was the bug — a teal bar under an orange "60".
  good: { color: "rgb(var(--wm-caution))", opacity: 0.55 },
  marginal: { color: "rgb(var(--wm-marginal))", opacity: 0.8 },
};

/**
 * Ring / number colour for one score — same band as SCORE_FILL and the
 * GO / MAYBE / NO verdict tones (accent / caution / marginal).
 */
export const scoreColor = (score) => {
  if (score === null || score === undefined) return "rgb(var(--wm-marginal))";
  if (score >= 75) return "rgb(var(--wm-accent))";
  if (score >= 60) return "rgb(var(--wm-caution))";
  return "rgb(var(--wm-marginal))";
};

export const scoreTextClass = (score) => {
  if (score === null || score === undefined) return "text-marginal";
