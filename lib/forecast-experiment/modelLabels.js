...
/** Windy-style label for a resolved forecast model slug (e.g. icon-global → ICON13). */
export function forecastModelWindyLabel(modelSlug) {
  const entry = FX_MODELS.find((model) => model.model === modelSlug);
  return entry?.windyLabel ?? modelSlug;
}
