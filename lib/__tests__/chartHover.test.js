...
describe("windHoverRows", () => {
  ...
  it("adds a Wave line when swell is present", () => {
    expect(
      windHoverRows({ forecastSpeed: 12, forecastGust: 16, waveHeight: 1.27 }).map(
        (r) => r.text
      )
    ).toEqual(["Forecast 12kt (16*)", "Wave 1.3m"]);
  });
});
...
describe("columnHoverCard", () => {
  ...
  it("includes Wave when the slot has swell", () => {
    const card = columnHoverCard(
      { hour: 16, slot: { timestamp: T0, speed: 18, gust: 22, waveHeight: 1.25 } },
      null,
      T0 + HOUR
    );
    expect(card.rows.map((r) => r.text)).toEqual([
