...
export function listModelsForSkillAnalysis(forecastPoints, { previousDayOnly = true } = {}) {
  ...
  const filtered = models.filter((model) => /-previous-day\d+$/.test(model));
...
export function analyzeModelSkill({
  ...
  const byModel = {};
  const scatterByModel = Object.fromEntries(models.map((model) => [model, []]));
  ...
  for (const model of models) {
    ...
    byModel[model] = {
      ...
      curve: aggregateCurveMetrics(dailyCurveSamples),
    };
  }

  return {
    models: rankModels(byModel),
    curveModels: rankModelsByCurve(byModel),
    byModel,
    totals: summarizeRegimeCounts(byModel),
    chartData: buildSkillChartData(scatterByModel, daySeriesByDate, models),
  };
}
