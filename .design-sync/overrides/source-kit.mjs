// forked from design-sync lib/source-kit.mjs — components/ui holds this repo's design-system primitives, so 'ui' must not be treated as a generic container dir (it would group all 24 primitives as 'general').
...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      // The repo's directory layout is NOT this design system's taxonomy.
      // components/ maps to how the app is organised (now/, next/, ui/,
      // forecast/); the design system needs the Current-vs-Legacy split the UI
      // kit publishes, which is what the `category:` frontmatter in
      // .design-sync/docs/<Name>.md carries.
      //
      // package-build only lets a doc category win when the group is falsy,
      // 'general' or 'misc' (it refuses to override a real source-derived
      // group), so the source-derived value is kept as `srcGroup` for
      // diagnostics and the group itself is left at 'general' for the docs to
      // claim. A component that lands in the "general" group in the output has
      // a MISSING or unreadable docs stub — that is the signal to look for.
      c.srcGroup = slug(
