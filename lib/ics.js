...
export function formatEventSummary(event) {
  ...
  let conditions = '';
  if (event.sport === 'wingfoil' || event.sport === 'kitesurfing') {
...
export function formatEventDescription(event) {
  ...
  const sportPath = event.sport === 'wingfoil' ? 'wing' : event.sport === 'kitesurfing' ? 'kite' : 'surf';
...
export function generateICS({ events, calendarName, calendarDescription }) {
  ...
  for (const event of events) {
    ...
    const sportName = event.sport === 'wingfoil' ? 'Wingfoiling' : event.sport === 'kitesurfing' ? 'Kitesurfing' : 'Surfing';
