...
/**
 * One-line summary for lists and hero cards.
 *
 * "26 kn NNW" in a list; "26 kn (33*) NNW" where the gust matters — the hero
 * card is someone deciding whether to drive, and gust range is part of that
 * decision in a way it is not when scanning a list.
 */
export function conditionSummary(slot, sport, { gust = false } = {}) {
  const m = primaryMetric(slot, sport);
  if (!m) return null;

  const parts = [`${m.value} ${m.unit}`];
  // For wind sports `secondary` IS the gust, in "(33*)" form.
  if (gust && m.secondary?.startsWith("(")) parts.push(m.secondary);
