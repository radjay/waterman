RatingInput from waterman. Use via `window.Waterman.RatingInput` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ConvexProvider>` (full provider chain in README.md — components read theme/i18n from that context).

## Props

```ts
interface RatingInputProps {
value: unknown; onChange: (...args: any[]) => void; disabled?: boolean;
}
```
