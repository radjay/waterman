...
function hourlyMaxEffective(marinaObservations, dateLocal) {
  const { startAt } = localDayWindowMs(dateLocal, DEFAULT_TIMEZONE);
  const maxByHour = {};
  for (let hour = FEATURE_HOUR_START; hour <= FEATURE_HOUR_END; hour += 1) {
    ...
    maxByHour[hour] = inHour.reduce((peak, obs) => {
...
/** Sustained hourly rideability — matches kick-in label + timelineSustainedKickIn logic. */
function hourlyRideableLabels(marinaObservations, dateLocal, thresholdKnots) {
  const maxByHour = hourlyMaxEffective(marinaObservations, dateLocal);
  const labels = {};
  for (let hour = FEATURE_HOUR_START; hour <= FEATURE_HOUR_END; hour += 1) {
    const rideableNow = maxByHour[hour] >= thresholdKnots;
    const rideableNext =
      hour < FEATURE_HOUR_END ? maxByHour[hour + 1] >= thresholdKnots : false;
    labels[`h${hour}Rideable`] = rideableNow && rideableNext ? 1 : 0;
  }
  return labels;
}
