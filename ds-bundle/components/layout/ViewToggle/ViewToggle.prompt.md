ViewToggle from waterman. Use via `window.Waterman.ViewToggle` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ConvexProvider>` (full provider chain in README.md — components read theme/i18n from that context).
...
compact?: boolean; rightContent: unknown; className?: string;
}
```
