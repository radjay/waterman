...
describe("parseCurrentReading", () => {
  ...
  // Number(null) is 0, and Number.isFinite(0) is true — a naive numeric
  // parse would turn an explicit null into a fabricated compass bearing and
  // a fabricated temperature. wind_direction and temperature are both
  // explicitly null on a genuine calm reading, not merely absent.
  it("treats an explicit null direction and temperature as absent, not zero", () => {
    const reading = parseCurrentReading(
      { ...LIVE, wind_direction: null, temperature: null },
      { nowMs: NOW }
    );
    expect(reading).not.toBeNull();
    expect(reading.direction).toBeUndefined();
  ...
  // Cabo Raso (3294) has been observed returning wind_avg: -534.6 and
  // wind_direction: -990 — Windguru sentinels, not a real storm. The live
  // cam was painting "-535" until we refused these.
  it("rejects negative / sentinel wind speeds", () => {
    expect(
      parseCurrentReading(
        {
          ...LIVE,
