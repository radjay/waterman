 ...
 *
 * Orange (`caution`) is MAYBE only. `--wm-marginal` is the same orange, so
 * scores below 60 use dim grey — a 0 must not read as MAYBE.
 *
 *   <60   dim grey        — NO
 *   60-74 caution @ .55   — MAYBE
 *   75-85 accent  @ .62   — GO
...
export const SCORE_FILL = {
  ...
  // Dim, not --wm-marginal: that token is the same orange as caution.
  marginal: { color: "var(--wm-dim)", opacity: 0.85 },
};

/**
 * Ring / number colour for one score — same band as SCORE_FILL and the
 * GO / MAYBE / NO verdict tones (accent / caution / dim).
 */
export const scoreColor = (score) => {
  if (score === null || score === undefined) return "var(--wm-dim)";
