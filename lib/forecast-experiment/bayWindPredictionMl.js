...
import { resolveKickInInRidingWindow, ridingWindowBounds } from "./ridingWindow.js";
...
function featureVectorToMap(modelFeatureNames, featureVector) {
  const named = featureVectorByName(featureVector);
  const map = {};
  for (const name of modelFeatureNames) {
    map[name] = named[name] ?? 0;
  }
  return map;
}
...
export function resolveNowcastCalibration(model, thresholdKnots) {
  const byThreshold = model.nowcastCalibration?.byThresholdKnots;
  if (byThreshold) {
    const tuned =
      byThreshold[String(thresholdKnots)] ?? byThreshold[thresholdKnots];
    if (tuned) return { ...DEFAULT_CALIBRATION, ...tuned };
  }

  const base = resolveMlCalibration(model, thresholdKnots) || DEFAULT_CALIBRATION;
  // Tuned for same-day tightening: slightly lower kick-in gate than day-ahead.
  const tunedKickIn = {
    10: 0.45,
    12: 0.45,
    15: 0.5,
  ...
  return {
    ...base,
    kickInThreshold: tunedKickIn[t] ?? 0.45,
    probabilityDamping: Math.min(base.probabilityDamping ?? 0.85, 0.9),
  };
}

export function resolveMlModelVersion(model) {
  if (model?.modelVersion?.includes("nowcast")) return model.modelVersion;
  if (model?.calibration) return model.modelVersion ?? "bay-wind-v3.6-ml";
  return model?.modelVersion ?? "bay-wind-v3-ml";
}
...
export function buildBayWindPredictionV3({
  ...
  const activeModel = isNowcast && nowcastModel ? nowcastModel : model;
  const modelVersion = resolveMlModelVersion(activeModel);
  ...
  const calibration = conservative
    ...
    : isNowcast
      ? resolveNowcastCalibration(activeModel, resolvedThreshold)
  ...
  const featureVector = buildMlFeatureVector({
    ...
    cutoffHourLocal,
    nowcastMode: isNowcast,
    referenceAt: generatedAt,
  });

  const sessionThreshold = calibration?.sessionThreshold ?? DEFAULT_CALIBRATION.sessionThreshold;
  const sessionProbability = predictRideableDayProbability(activeModel, featureVector);
  const sessionAllowed = !activeModel.rideableDayClassifier || sessionProbability >= sessionThreshold;

  const kickInMinutes = sessionAllowed ? predictKickInMinutes(activeModel, featureVector) : undefined;
  const hourlyRaw = predictHourlyRideableProbabilities(activeModel, featureVector, calibration);
  const kickInThreshold = calibration?.kickInThreshold ?? 0.5;
  ...
  const { windowStart, windowEnd } = ridingWindowBounds(forecastDateLocal, DEFAULT_TIMEZONE);
  ...
  const predictedStrongKickInAt = sessionAllowed
    ? probabilityTimeline.find(
        (row) =>
          row.time >= windowStart &&
          row.time <= windowEnd &&
          row.rideableProbability >= Math.min(0.95, kickInThreshold + 0.15)
      )?.time
    : undefined;
  ...
  return {
    ...
    probabilityTimeline,
    confidence,
    summary: summaryV3({ predictedKickInAt, predictedStrongKickInAt, peak, kickInMinutes, modelVersion, sessionProbability }),
    inputs: {
...
function summaryV3({ predictedKickInAt, predictedStrongKickInAt, peak, kickInMinutes, modelVersion, sessionProbability }) {
  ...
  const kickIn = new Date(predictedKickInAt).toISOString();
  const strongKickIn = predictedStrongKickInAt ? new Date(predictedStrongKickInAt).toISOString() : "not reached";
  ...
  return `Bay rideability crosses 50% near ${kickIn}; strong threshold: ${strongKickIn}; peak near ${peakIso}.${minutesNote}`;
}
