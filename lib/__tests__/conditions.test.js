...
describe("conditionSummary — gusts", () => {
  it("includes the gust when asked, between the speed and the direction", () => {
    // The hero card is someone deciding whether to drive; gust range is part
    // of that call in a way it is not when scanning a list.
    expect(conditionSummary(slot(), "wingfoil", { gust: true })).toBe("22 kn (27*) NNW");
  });

  it("still omits it by default", () => {
    expect(conditionSummary(slot(), "wingfoil")).toBe("22 kn NNW");
  });

  it("does not inject a gust into a surf summary, where secondary is the period", () => {
    const s = conditionSummary(slot(), "surfing", { gust: true });
    expect(s).toMatch(/^1\.4 m .* swell @ 11 s$/);
