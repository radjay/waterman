...
// `direction` is the RAW stored bearing. Config ranges are authored against
// the DISPLAYED bearing (raw + 180), so a raw 150 displays as 330 — inside the
// 315->45 range below.
const slot = (speed, gust = speed + 4, direction = 150) => ({ speed, gust, direction });
...
describe("modelVote", () => {
  it("passes a slot that clears speed, gust and direction", () => {
    expect(modelVote(slot(20, 25, 150), WING)).toBe(true);
  });

  it("fails on speed, gust or direction independently", () => {
    expect(modelVote(slot(10, 25, 150), WING)).toBe(false);
    expect(modelVote(slot(20, 12, 150), WING)).toBe(false);
    expect(modelVote(slot(20, 25, 0), WING)).toBe(false); // displays as 180, out of range
  });

  it("handles the wrap-around direction range", () => {
    // 315 -> 45 crosses north, in DISPLAYED bearings.
    expect(modelVote(slot(20, 25, 190), WING)).toBe(true); // displays 10
    expect(modelVote(slot(20, 25, 170), WING)).toBe(true); // displays 350
    expect(modelVote(slot(20, 25, 20), WING)).toBe(false); // displays 200
  });
...
const modelSlots = (speeds) =>
  WIND_MODELS.map((model, i) => ({ model, slot: slot(speeds[i], speeds[i] + 5, 150) }));
...
describe("agreementFor", () => {
  ...
  it("scales the good threshold to however many models actually returned", () => {
    ...
    const three = (speeds) =>
      speeds.map((s, i) => ({ model: WIND_MODELS[i], slot: slot(s, s + 5, 150) }));
...
describe("direction convention", () => {
  // spotConfigs ranges are authored against the bearing the UI displays, which
  // is the stored bearing + 180. Getting this backwards is silent: every vote
  // fails and the grid reports unanimous disagreement nobody expressed.
  const marina = { minSpeed: 15, minGust: 18, directionFrom: 315, directionTo: 135 };

  it("passes a slot whose DISPLAYED direction is in range", () => {
    // Stored 156 displays as NNW (336), which is inside 315->135.
    expect(modelVote({ speed: 20, gust: 25, direction: 156 }, marina)).toBe(true);
  });

  it("fails a slot whose displayed direction is outside the range", () => {
    // Stored 20 displays as 200 (SSW), outside 315->135.
    expect(modelVote({ speed: 20, gust: 25, direction: 20 }, marina)).toBe(false);
  });

  it("does not compare the raw stored bearing against the config", () => {
    // The regression: raw 156 is NOT inside 315->135, so a raw comparison
    // rejected a slot the app itself renders as a perfect NNW.
    const rawWouldReject = { speed: 20, gust: 25, direction: 156 };
    expect(modelVote(rawWouldReject, marina)).not.toBe(false);
  });
});

describe("threshold calibration", () => {
  // spotConfigs stores minSpeed 15 for every spot, a default nobody tuned. The
  // scorer plainly uses a different, spot-specific bar — so voting models
  // against 15 reported "0 of 5" on slots the app itself scored 85.
  const scored = (pairs) => pairs.map(([speed, score]) => ({ speed, gust: speed + 4, score }));

  it("learns a lower bar when the spot's own scores show one", () => {
    const slots = scored([
      [10.5, 68], [11, 72], [12, 78], [13, 80], [14, 85], [15, 88], [16, 90],
      [6, 20], [7, 30],
    ]);
    const t = calibrateThreshold(slots);
    expect(t.calibrated).toBe(true);
    expect(t.minSpeed).toBeLessThan(15);
    expect(t.minSpeed).toBeGreaterThan(9);
  });

  it("ignores one oddly-scored slot rather than dropping the bar to the floor", () => {
    const slots = scored([
      [3, 65], [12, 70], [13, 75], [14, 80], [15, 85], [16, 88], [17, 90],
    ]);
    expect(calibrateThreshold(slots).minSpeed).toBeGreaterThan(5);
  });

  it("refuses to calibrate from too few samples", () => {
    expect(calibrateThreshold(scored([[12, 70], [13, 75]]))).toBeNull();
  });

  it("refuses to calibrate when nothing scored well", () => {
    expect(calibrateThreshold(scored([[4, 10], [5, 20], [6, 15], [3, 8], [5, 12], [4, 9]]))).toBeNull();
  });

  it("prefers the calibrated bar over the stored config", () => {
    const slots = scored([
      [10.5, 68], [11, 72], [12, 78], [13, 80], [14, 85], [15, 88], [16, 90],
    ]);
    const t = thresholdFor(WING, "wingfoil", slots);
    expect(t.minSpeed).toBeLessThan(WING.minSpeed);
    expect(t.isDefault).toBe(false);
  });

  it("keeps the config's direction range, which scores cannot teach", () => {
    const slots = scored([
      [10.5, 68], [11, 72], [12, 78], [13, 80], [14, 85], [15, 88], [16, 90],
    ]);
    const t = thresholdFor(WING, "wingfoil", slots);
    expect(t.directionFrom).toBe(WING.directionFrom);
    expect(t.directionTo).toBe(WING.directionTo);
  });

  it("falls back to the config when calibration is not possible", () => {
    expect(thresholdFor(WING, "wingfoil", []).minSpeed).toBe(WING.minSpeed);
    expect(thresholdFor(WING, "wingfoil").minSpeed).toBe(WING.minSpeed);
  });

  it("still refuses to calibrate a surf threshold from wind speeds", () => {
    const slots = scored([
      [10.5, 68], [11, 72], [12, 78], [13, 80], [14, 85], [15, 88], [16, 90],
    ]);
    expect(thresholdFor(WING, "surfing", slots)).toBeNull();
  });
});
