  ...
  Object.keys(grouped).forEach((day) => {
    Object.keys(grouped[day]).forEach((spotId) => {
      ...
      for (const sport of selectedSports) {
        // Find contextual slot for this sport (closest before sunrise or after sunset)
        slots.forEach(slot => {
      ...
      // Also mark slots where sunset occurs in the first half as contextual
      // These slots aren't ideal for sessions since sunset happens early in the slot
      slots.forEach(slot => {
        if (!slot.isContextual && sunsetOccursInFirstHalfOfSlot(new Date(slot.timestamp), spot)) {
          slot.isContextual = true;
          // Don't set contextualSport since this is a different type of contextual slot
        }
      });
    });
  });
