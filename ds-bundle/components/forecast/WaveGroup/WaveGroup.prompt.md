WaveGroup from waterman. Use via `window.Waterman.WaveGroup` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ConvexProvider>` (full provider chain in README.md — components read theme/i18n from that context).

## Props

```ts
interface WaveGroupProps {
waveHeight: unknown; wavePeriod: unknown; waveDirection: unknown; className?: string;
}
```
