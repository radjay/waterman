...
describe("scoreColor / scoreTextClass — same band as GO / MAYBE / NO", () => {
  ...
  it("uses marginal below the bar", () => {
    expect(scoreColor(59)).toContain("--wm-marginal");
    expect(scoreTextClass(41)).toBe("text-marginal");
  });
});

describe("VERDICT_TONE tracks the same bands as the ring", () => {
  it("paints NO with the marginal token, not dim", async () => {
    const { VERDICT, VERDICT_TONE } = await import("../verdict");
    expect(VERDICT_TONE[VERDICT.NO]).toBe("marginal");
