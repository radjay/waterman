...
page.on("requestfailed", (r) => {
  failedCount++;
  console.log(
    `   Failed: ${r.url().substring(0, 80)} - ${r.failure()?.errorText}`
  );
});
...
try {
  ...
  console.log(
    `   Requests: ${requestCount}, Responses: ${responseCount}, Failed: ${failedCount}`
  );
  ...
  console.log(
    `   Requests made: ${requestCount}, Responses: ${responseCount}, Failed: ${failedCount}`
  );
