 ...
 *
 * @returns {boolean|null} null means "cannot say" (no usable threshold)
 */
export function modelVote(slot, threshold) {
  ...
  const speedOk = (slot.speed ?? 0) >= threshold.minSpeed;
  const gustOk = (slot.gust ?? 0) >= threshold.minGust;
  ...
  return speedOk && gustOk && directionOk;
}
...
export function agreementFor(modelSlots, threshold) {
  ...
  if (total === 0) {
    return { band: BANDS.UNKNOWN, agreed: 0, total: 0, models: [], outlier: null };
  }

  const agreed = votes.filter((v) => v.vote).length;

  // GOOD needs a strong majority: 4 of 5.
  const goodThreshold = Math.ceil(0.8 * total);
  let band;
  if (agreed >= goodThreshold) band = BANDS.GOOD;
  else if (agreed >= 2) band = BANDS.SPLIT;
  else band = BANDS.NO;
  ...
  let outlier = null;
  if (agreed === total - 1) outlier = votes.find((v) => !v.vote)?.model ?? null;
  else if (agreed === 1) outlier = votes.find((v) => v.vote)?.model ?? null;
