...
export function scoreGuinchoModelSkill({ observations, openMeteoPoints, windySlots = [] }) {
  ...
  const allFingerprints = [...hoursByDate.entries()]
    .map(([dateLocal, hours]) => fingerprintDay(dateLocal, hours, forecastIndex, 1, { models: day1Peers }))
    .filter(Boolean);
