...
describe("scoreColor / scoreTextClass — same band as GO / MAYBE / NO", () => {
  it("uses accent for GO scores (≥75)", () => {
    expect(scoreColor(75)).toContain("--wm-accent");
    expect(scoreColor(90)).toContain("--wm-accent");
    expect(scoreTextClass(80)).toBe("text-accent");
  });

  it("uses caution for MAYBE scores (60-74), not accent", () => {
    // Regression: a MAYBE 60 used to paint a teal ring beside an orange label.
    expect(scoreColor(60)).toContain("--wm-caution");
    expect(scoreColor(74)).toContain("--wm-caution");
    expect(scoreTextClass(60)).toBe("text-caution");
    expect(SCORE_FILL.good.color).toContain("--wm-caution");
  });

  it("uses dim grey below 60 — orange is MAYBE only", () => {
    expect(scoreColor(0)).toBe("var(--wm-dim)");
    expect(scoreColor(59)).toBe("var(--wm-dim)");
    expect(scoreTextClass(0)).toBe("text-dim");
    expect(scoreTextClass(41)).toBe("text-dim");
    expect(SCORE_FILL.marginal.color).toBe("var(--wm-dim)");
  });
});

describe("VERDICT_TONE tracks the same bands as the ring", () => {
  it("paints NO dim, MAYBE caution, GO accent", async () => {
    const { VERDICT, VERDICT_TONE } = await import("../verdict");
    expect(VERDICT_TONE[VERDICT.NO]).toBe("dim");
    expect(VERDICT_TONE[VERDICT.MARGINAL]).toBe("caution");
    expect(VERDICT_TONE[VERDICT.GO]).toBe("accent");
  });
});
