 ...
 * aligned. Labels like "25kt" and "1.3m" sit in this gutter; bars and lines
...
export function windScale(values = [], { min = 30, step = 10 } = {}) {
  ...
  // Two reference lines, at ~5/6 and ~1/2 of the scale, on round knots.
  const upper = Math.round((max * 5) / 6 / 5) * 5;
  const mid = Math.round(max / 2 / 5) * 5;
  const lines = [upper, mid]
    .filter((v, i, a) => v > 0 && v < max && a.indexOf(v) === i)
    .map((v) => ({ value: v, label: `${v}kt`, top: (1 - v / max) * 100 }));
  return { max, lines };
}

/**
 * Wave y-scale: 0..max metres, with one reference line at half scale.
 *
...
export function waveScale(values = [], { min = 1, step = 0.5 } = {}) {
  ...
  return {
    max,
    lines: mid > 0 && mid < max ? [{ value: mid, label: `${mid}m`, top: (1 - mid / max) * 100 }] : [],
  };
}
