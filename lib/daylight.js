...
/**
 * Check if a timestamp is clearly nighttime (for safety filtering)
 * This is a conservative check that excludes obviously nighttime hours
 * @param {Date} timestamp - The slot timestamp
 * @returns {boolean} True if slot is clearly nighttime
 */
export function isNighttimeSlot(timestamp) {
    const hour = timestamp.getHours();
    // Exclude hours that are clearly nighttime (10 PM - 6 AM)
 ...
 */
export function isDaylightSlot(timestamp, spot, slotDurationHours = 3) {
    ...
    const slotStart = timestamp.getTime();
    const slotEnd = slotStart + (slotDurationHours * 60 * 60 * 1000);

    // A slot is daylight if its time window overlaps with daylight at all.
    // Only filter out slots entirely before sunrise or entirely after sunset.
    return slotEnd > sunTimes.sunrise.getTime() && slotStart < sunTimes.sunset.getTime();
}
...
export function isAfterSunset(timestamp, spot) {
    ...
    const sunTimes = getSunTimes(spot.latitude, spot.longitude, timestamp);
    // Only "after sunset" if the slot starts after sunset
    return timestamp.getTime() >= sunTimes.sunset.getTime();
}
...
/**
 * True when it is too dark to ride at this spot right now.
 *
 * After sunset, or any time before sunrise (the whole pre-dawn dark window —
 * not only the deep night), you cannot surf, kite, or wing. Daylight between
 * sunrise and sunset is rideable light; civil-twilight edge cases still use
 * the real sun times for the spot.
 *
 * Falls back to 22:00–06:00 local clock when the spot has no coordinates
 * (Praia do CDS / Fonte da Telha still have coords in practice; this is the
 * safety net).
 *
 * @param {object|null} spot - with latitude/longitude when available
 * @param {number|Date} [now] - ms or Date; defaults to now
 * @returns {boolean}
 */
export function isDarkForSession(spot, now = Date.now()) {
    ...
    const sunTimes = getSunTimes(spot.latitude, spot.longitude, at);
    ...
    // Before sunrise (entire pre-dawn window) or at/after sunset.
    return ms < sunrise || ms >= sunset;
}
