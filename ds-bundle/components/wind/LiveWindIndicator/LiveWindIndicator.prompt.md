LiveWindIndicator from waterman. Use via `window.Waterman.LiveWindIndicator` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<ConvexProvider>` (full provider chain in README.md — components read theme/i18n from that context).

Compact live-wind badge for spot headers and webcam overlays.
...
```ts
interface LiveWindIndicatorProps {
